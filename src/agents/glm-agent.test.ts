 ...
 * tokens count against max_tokens — omitting the parameter caused production empty-response
...
function makeAgent(completion: any): { agent: GlmAgent; captured: { params?: any } } {
  // Real catalog model id so the agent resolves reasoningEffort from the catalog.
  const agent = new GlmAgent('Mira', 'instruction', 'glm-5.3', 'key', 0.7, false, SILENT_LOGGING);
...
describe('GlmAgent request shape', () => {
  ...
  it('a per-instance override replaces the catalog effort', async () => {
    const { agent, captured } = makeAgent(textCompletion);
