...
/**
 * Pure framing geometry, shared by the slicer (server), reframe validation (server)
 * and renderers/editors (browser). No I/O here.
 *
 * Coordinates: a card is an ImageRect in SHEET pixels (portrait 3:4); the circle is
 * card-relative (x, d = fractions of the card's width, y = fraction of its height).
 * An ImageFocus is what a renderer needs: the fraction of an image to show, so the
 * same avatar component can show a circle cut out of a stored card OR out of a
 * static preset sheet.
 */
...
/** Keeps the circle inside its card. */
...
/** The circle as a focus on the SHEET (card and circle composed) — how a preset
 * mannequin renders straight off a static sheet. */
